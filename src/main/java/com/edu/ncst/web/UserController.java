...
import com.edu.ncst.model.User;
import com.edu.ncst.service.UserService;
import com.edu.ncst.utils.Md5Util;
import com.edu.ncst.utils.MyResult;
import com.edu.ncst.utils.MyUtils;
import com.edu.ncst.utils.TokenProcessor;
...
public class UserController {
    ...
    public Map<String, Object> login(@RequestBody User user) {
        //开始改动
        String Md5Password = Md5Util.getMD5String(user.getUserpassword());
...
//    }
    ...
    @RequestMapping(value = "/register")
    //参数校验
    public Integer register(@Pattern(regexp = "^\\S{1,10}$") String username, @Pattern(regexp = "^\\S{1,10}$") String password) {
        //密码加密
        String md5String = Md5Util.getMD5String(password);
        //保存
        return userService.register(username, md5String);
    }
    //改动结束
...
// 修改密码
    ...
    public Integer alterPassword(Integer userid, String username, Byte isadmin, String oldPassword, String newPassword) {
        //对输入的旧密码进行加密
        //输入的旧密码加密
        String oldMd5String = Md5Util.getMD5String(oldPassword);
        //检查旧密码是否正确
        User userObj = new User();
        userObj.setUserid(userid);
        userObj.setUsername(username);
        userObj.setUserpassword(oldMd5String);
        userObj.setIsadmin(isadmin);
        User user = userService.login(userObj);
        if (user == null) {  //旧密码不正确
            return 0;
            ...
            userService.setPassword(userObj.getUserid(), Md5String);
            return 1;
        }
    }
    //改动结束
